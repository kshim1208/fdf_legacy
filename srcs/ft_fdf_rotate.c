...
/*   Updated: 2022/08/16 17:33:00 by kshim            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_fdf_isomet_project(t_ft_fdf_data_set *set)
{
	set -> prg -> degree[0] = 45;
	set -> prg -> degree[1] = 0;
	set -> prg -> degree[2] = 180; /* 180 + 35.24*/ ;
	return ;
}
...
void	ft_fdf_rotate_axis_x(t_ft_fdf_pnt_data *draw, t_ft_fdf_data_set *set)
{
	...
	draw -> y = (
			cos((pi / 180) * rot) * (org_y - (set -> prg -> axis_pnt[1])) + (
				(-1 * sin((pi / 180) * rot))) * (org_z - (set -> prg -> axis_pnt[2])))
		+ set -> prg -> axis_pnt[1];
	draw -> z = (
			sin((pi / 180) * rot) * (org_y - (set -> prg -> axis_pnt[1])) + (
				cos((pi / 180) * rot)) * (org_z - (set -> prg -> axis_pnt[2])))
		+ set -> prg -> axis_pnt[2];
	return ;
}
...
void	ft_fdf_rotate_axis_y(t_ft_fdf_pnt_data *draw, t_ft_fdf_data_set *set)
{
	...
	pi = 3.14159265358979323846;
	draw -> x = (
			cos((pi / 180) * rot) * (org_x - (set -> prg -> axis_pnt[0])) + (
				sin((pi / 180) * rot)) * (org_z - (set -> prg -> axis_pnt[2])))
		+ set -> prg -> axis_pnt[0];
	draw -> z = (
			(-1 * sin((pi / 180) * rot)) * (org_x - (set -> prg -> axis_pnt[0])) + (
				cos((pi / 180) * rot)) * (org_z - (set -> prg -> axis_pnt[2])))
		+ set -> prg -> axis_pnt[2];
	return ;
}
...
void	ft_fdf_rotate_axis_z(t_ft_fdf_pnt_data *draw, t_ft_fdf_data_set *set)
{
	...
	draw -> x = (
			cos((pi / 180) * rot) * (org_x - (set -> prg -> axis_pnt[0])) + (
				(-1 * sin((pi / 180) * rot))) * (org_y - (set -> prg -> axis_pnt[1])))
		+ set -> prg -> axis_pnt[0];
	draw -> y = ((
				sin((pi / 180) * rot) * (org_x - (set -> prg -> axis_pnt[0])) + (
					cos((pi / 180) * rot)) * (org_y - (set -> prg -> axis_pnt[1])))
			+ set -> prg -> axis_pnt[1]);
	return ;
}
